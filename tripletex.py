...
# Employee
    ...
    def create_employee(self, payload):
        ...
        r = requests.post(f'{self.base_url}/employee', data=json.dumps(payload), auth=self.auth, headers=header)
        return self.map(r)
...
# project
    ...
    def create_project(self, payload):
        header = {'Content-Type': 'application/json' }
        r = requests.post(f'{self.base_url}/project', data=json.dumps(payload), auth=self.auth, headers=header)      
        return self.map(r)

    def update_project(self, id, payload):
        header = {'Content-Type': 'application/json' }
        r = requests.put(f'{self.base_url}/project/{id}', data=json.dumps(payload), auth=self.auth, headers=header)
        return self.map(r)
...
# order
    ...
    def create_order(self, payload):
        header = {'Content-Type': 'application/json' }
        r = requests.post(f'{self.base_url}/order', data=json.dumps(payload), auth=self.auth, headers=header)
        return self.map(r)
...
# product
    ...
    def create_product(self, payload):
        header = {'Content-Type': 'application/json' }
        r = requests.post(f'{self.base_url}/product', data=json.dumps(payload), auth=self.auth, headers=header)      
        return self.map(r)

    def get_products(self):
        header = {'Content-Type': 'application/json' }
        r = requests.get(f'{self.base_url}/product', auth=self.auth, headers=header)      
        return self.map(r)   
 
 # customer
    def create_customer(self, payload):
        header = {'Content-Type': 'application/json' }
        r = requests.post(f'{self.base_url}/customer', data=json.dumps(payload), auth=self.auth, headers=header)
        return self.map(r)
...
# invoice
    def create_invoice(self, payload):
        header = {'Content-Type': 'application/json' }
        r = requests.post(f'{self.base_url}/invoice', data=json.dumps(payload), auth=self.auth, headers=header)
        return self.map(r)
